...
void  SetPCM_InitConnectTb(uint32_t count){
    if(count==1){
        SCU->DATA[1] = 0xE01C; //PCM con TB
        {
            PADConfig_Type pad_config;
            memset(&pad_config,0,sizeof(PADConfig_Type));
            pad_config.mode = 1;
            SystemPADConfig(PADID19,&pad_config);
            SystemPADConfig(PADID20,&pad_config);
            SystemPADConfig(PADID21,&pad_config);
            SystemPADConfig(PADID22,&pad_config);
        }
    }
}
...
void VerifyMain(uint32_t count)
{
    ...
    if (count == 25) 
    {
        ...
        struct ld_test_params par =
        {
            ...
            .pkt_tcod = TST_EV3_2_TYPE,
            .pl_len   = 0,
            .channel  = 0,
            .lt_addr  = 1,
            .pwr_lvl  = RF_POWER_MAX,
            .random   = LD_RAND_MOD_PCM,
            .pattern  = 0x55,
        };
